...
(() => {
  ...
  class liquidGLRenderer {
    constructor(snapshotSelector, snapshotResolution = 1.0) {
      ...
      const scrollCheck = () => {
        ...
        this._scrollCheckRaf = requestAnimationFrame(scrollCheck);
      };
      this._scrollCheckRaf = requestAnimationFrame(scrollCheck);
      ...
      this._dynamicStyleElement = styleEl;
    ...
    /* --------------------------------------------------
     *  Target-lab lifecycle extension.
     *
     *  This is the only behavioral addition to the authoritative renderer:
     *  the standalone demo lives for the lifetime of a page, while a saved
     *  Layer C renderer must be fully removable when another save is selected.
     * ------------------------------------------------*/
    destroy() {
      if (this._destroyed) return;
      ...
      if (this._scrollCheckRaf) cancelAnimationFrame(this._scrollCheckRaf);
      if (this._scrollTimeout) clearTimeout(this._scrollTimeout);
      if (this._onResize) window.removeEventListener("resize", this._onResize);
      if (this._snapshotResizeObserver) this._snapshotResizeObserver.disconnect();

      this.lenses.forEach((lens) => lens.destroy && lens.destroy());
      ...
      if (this._dynamicStyleElement) this._dynamicStyleElement.remove();
    }
  }
  ...
  class liquidGLLens {
    ...
    _reveal() {
      ...
      const animate = () => {
        if (this._destroyed || this.renderer._destroyed) return;
        ...
        if (progress < 1) {
          this._revealRaf = requestAnimationFrame(animate);
        } else {
    ...
    /* See liquidGLRenderer.destroy(): standalone liquidGL never needs this,
       but selectable Layer C instances do. */
    destroy() {
      if (this._destroyed) return;
      this._destroyed = true;
      ...
      if (this._tiltHandlersBound) this._unbindTiltHandlers();
      this._destroyMirrorCanvas();
  ...
  /* Target-lab lifecycle extension; see renderer.destroy above. */
