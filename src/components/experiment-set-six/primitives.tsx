...
function ExperimentSixLayerBSheet() {
  const { e6 } = useExperimentSetOne();
  return (
    <div
      className="experiment-four-layer-b"
      role="region"
      aria-label="Experiment Six layer B"
      style={e4LayerBDimensionStyle(e6, false)}
    ...
    >
      ...
      <div className="experiment-four-layer-b__content">
        <LayerCopy
          ...
          title="Layer B"
          subtitle="Experiment Six · nested frost body"
          body="Compact nested panel — layer C sits on top and can be dragged within this inset."
        />
      </div>
    </div>
  );
...
function ExperimentSixLayerASheet({ layoutResetVersion = 0 }: { layoutResetVersion?: number }) {
  const { e6, e6LayerC, layerBVisible, layerCVisible } = useExperimentSetOne();
  const showInset = layerBVisible || layerCVisible;
  const layerCOffset = experimentSixLayerCDefaultOffset(e6LayerC, e6);

  return (
    <div
      className="experiment-four-layer-a"
      role="region"
      aria-label="Experiment Six layer A"
      style={e4LayerADimensionStyle(e6)}
    ...
    >
      ...
      <PwzzovOGlassCorners
        ...
        edgeReflexEnabled={layerBackdropReflexEnabled('layerA', e6)}
        rimSideGapTop={e6.layerARimSideGapTop}
        rimSideGapBottom={e6.layerARimSideGapBottom}
        backdropLights={layerBackdropLights('layerA', e6)}
      />
      {showInset && (
        <div className="experiment-four-layer-a__bezel-inset">
          {layerBVisible && <ExperimentSixLayerBSheet />}
          {layerCVisible && (
            <ExperimentSixLayerCDragInBezel
              initialPosition={layerCOffset}
      ...
      <div className="experiment-four-layer-a__content">
        <LayerCopy
          ...
          title="Layer A"
          subtitle="Experiment Six · compact nested bezel"
          body="Branch save materials with compact 316×97 geometry and forced nested bezel."
        />
      </div>
    </div>
  );
...
export function ExperimentSixDraggableLayerA({
  ...
  if (!layerAVisible && !layerBVisible && !layerCVisible) return null;

  return (
    <ExperimentTwoDraggableSheet
      initialPosition={initialPosition}
      ariaLabel="Experiment Six — composite nested panel"
      persistKey={persistKey}
      layoutResetVersion={layoutResetVersion}
    >
      <ExperimentSixLayerASheet layoutResetVersion={layoutResetVersion} />
    </ExperimentTwoDraggableSheet>
  );
}
